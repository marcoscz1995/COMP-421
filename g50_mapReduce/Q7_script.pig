...
--join
--get the airports codes for lax and jfk
deltajoin = JOIN delta BY tail_number, flights BY tail_number;
--get the flights that originate in either jkf or lax
jfklaxjoinOrigin = JOIN jfklaxids BY airport_id, deltajoin BY origin_airport_id; 
--get the flights that arrive in either jkf or lax
...
--order by totoaldistance then tail number, limit to 10
